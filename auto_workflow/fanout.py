...
from .build import current_build_context, TaskInvocation

class DynamicFanOut(list):  # placeholder container recognized by scheduler
    def __init__(self, task_def, source_invocation: TaskInvocation | 'DynamicFanOut', max_concurrency: int | None, ctx):
        super().__init__()
    ...
    def expand(self, values: Iterable[Any]):
        if self._expanded:
            return
        for v in values:
            self.append(self._ctx.register(self._task_def.name, self._task_def.fn, (v,), {}, self._task_def))
        self._expanded = True
...
def fan_out(task_def, iterable: Iterable[Any], *, max_concurrency: int | None = None) -> List[Any]:
    ...
    from .fanout import DynamicFanOut as DF  # self-import safe
    if isinstance(iterable, TaskInvocation):  # dynamic runtime fan-out
        ...
        return df
    if isinstance(iterable, DF):  # nested dynamic
