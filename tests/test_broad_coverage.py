import asyncio, time, json, os, textwrap, subprocess, sys
import pytest
from auto_workflow import task, flow, fan_out, FailurePolicy, subscribe
from auto_workflow.cache import get_result_cache
from auto_workflow.artifacts import get_store, ArtifactRef
from auto_workflow.secrets import StaticMappingSecrets, set_secrets_provider, secret, EnvSecrets
...
# 4. Cache key reuse
@task(cache_ttl=2)
def cached(x:int): return x*2

# 5. Artifact persistence memory
@task(persist=True)
def big(): return {"k":123}
...
# 6. Secret usage
@task
def read_secret()->str|None: return secret("API_KEY")
...
class Recorder:
    ...
    async def span(self,name:str,**attrs):
        self.names.append(name); yield
rec=Recorder(); set_tracer(rec)
...
def full_flow():
    ...
    b = flaky_once()
    try:
        sleeper()  # will timeout -> raise -> fail flow
    except Exception:
        pass
    c = cached(5)
    d = cached(5)
...
def test_retry_once(): assert flaky_once() == 42
...
def test_cache_reuse():
    r1 = cached(2); r2 = cached(2); assert r1 == r2 == 4


def test_artifact_memory():
    ref = big_flow.run(); store=get_store(); assert isinstance(ref,ArtifactRef); assert store.get(ref)["k"]==123
...
def test_priority_ties():
    f = export_flow.run()
    # just ensure both priority tasks definable and callable
...
def test_artifact_flow():
    ref = artifact_flow.run(); assert isinstance(ref,ArtifactRef)


def test_secret_flow_env(monkeypatch):
    os.environ['API_KEY']='xyz'; set_secrets_provider(EnvSecrets()); assert secret_flow.run()=="xyz"; del os.environ['API_KEY']


def test_export_dot_and_graph():
    dot = export_flow.export_dot(); graph = export_flow.export_graph(); assert 'add:1' in dot or 'add' in dot; assert isinstance(graph, dict)
